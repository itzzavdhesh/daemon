...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(axiosError.response?.data?.error || 'Unable to log in');
    } finally {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-6 py-12">
      <section className="w-full max-w-md rounded-lg border border-daemon-line bg-daemon-panel p-8 shadow-glow">
        ...
        <p className="mt-2 text-sm text-daemon-muted">Sign in to continue building state machines.</p>
        ...
        <p className="mt-6 text-center text-sm text-daemon-muted">
          New to Daemon?{' '}
          <Link className="font-semibold text-daemon-accent" href="/register">
            Create an account
          </Link>
        </p>
      </section>
